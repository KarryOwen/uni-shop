...
import type { LogisticItem } from '@/services/order'
...
// 获取物流信息
const logtics = ref<LogisticItem[]>()
const getorderLogistics = async () => {
  const ans = await getMemberOrderLogisticsByIdAPI(query.id)
  logtics.value = ans.result.list
}
// 获取订单状态
const order = ref<OrderResult>()
const getMemberOrder = async () => {
  const ans = await getMemberOrderByIdAPI(query.id)
  // console.log(ans)
  order.value = ans.result
  if (
...
<template>
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="true">
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logtics" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
